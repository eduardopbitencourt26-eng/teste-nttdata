...
final class ApiTokenService {
  ...
  private function makeKey(string $token): string {
    // chave derivada do token (hash), não armazenamos o token em claro
  ...
  public function issueForUid(int $uid, int $ttl = 3600, array $scopes = ['poll:read','poll:vote']): array {
    ...
    // O Expirable usa timestamp absoluto como expiração.
  ...
  /**
   * Valida token e (opcionalmente) checa escopo.
   * @return array{uid:int,scopes:array,created:int}|null
   */
  ...
  /** Revoga token atual (logout opcional) */
